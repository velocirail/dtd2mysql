...
describe("Association", () => {
  ...
  it("does not create a copy of the associated schedule for dates when it does not run", () => {
    ...
    const association1 = new Association(
      ...
      DateIndicator.Same,
      AssociationType.Split,
      new ScheduleCalendar(moment("2017-07-17"), moment("2017-08-16"), ALL_DAYS),
      STP.Overlay
    );
    ...
    chai.expect(results[0].tuid).to.equal("A_B");
    chai.expect(results[0].calendar.isEmpty).to.equal(false);
  });
  ...
  it("does not create a schedule for exclude days when the associated schedule does not run", () => {
    ...
    const association1 = new Association(
      ...
      DateIndicator.Same,
      AssociationType.Split,
      new ScheduleCalendar(moment("2017-07-10"), moment("2017-07-21"), ALL_DAYS, { "20170716": moment("2017-07-16") }),
      STP.Overlay
    );
